...
def scanline_convert(polygons, i, screen, zbuffer ):
    pointsofpoly = polygons[i:i+3]
    def sorter(b):
        return b[1]
    #ascending order
    pointsofpoly.sort(key = sorter)
    top = pointsofpoly[2]
    middle = pointsofpoly[1]
    bottom = pointsofpoly[0]
    ...
    for y in range (y0,ym):
        y = int(y)
        draw_line(int(x0),int(y),z0,int(x1),int(y),z1,screen,zbuffer,randcolor)
        if(top[1] != bottom[1]):
    ...
    for y in range(ym,y1):
        draw_line(int(x0),int(y),z0,int(xm),int(y),zm,screen,zbuffer,randcolor)
        if(top[1] != bottom[1]):
            x0 += (top[0]-bottom[0])/(top[1]-bottom[1])
            z0 += (top[2]-bottom[2])/(top[1]-bottom[1])
        if(top[1] != middle[1]):
            xm += (top[0]-middle[0])/(top[1]-middle[1])
            zm += (top[2]-middle[2])/(top[1]-middle[1])
